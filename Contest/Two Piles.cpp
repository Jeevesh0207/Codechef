Question Link :- (" https://www.codechef.com/problems/SPLITMIN?tab=statement ")
...
void Rabbit()
{
  ...
  FOR(i, 0, n)
  {
    ...
    pp[2*i+1].first = y;
    pp[2*i+1].second = i;
    base = max(base, min(x, y));
  }
  sort(pp, pp + 2 * n);
  ll ans = INF;
  FOR(i, 0, 2 * n - 1)
  {
    if (pp[i].first >= base || pp[i + 1].first >= base)
    {
      if (pp[i].second != pp[i + 1].second)
      {
        ans = min(ans, pp[i + 1].first - pp[i].first);
      }
    }
  }
  cout << ans << endl;
